...
### 追加：0.4.0 事故与 0.4.1 修复（2026-07-22 23:43）

- 0.4.0 发布链路本身验证通过（tag/手动触发 → CI → watchtower 5 分钟内自动更新 NAS，全程零手工）。
- **事故**：0.4.0 在 NAS 启动失败——v122 迁移的"一致性锚点"（2150股@112.572 等）是针对开发验收库的特定数据，却对任何非空 positions 的存量库强制核对，NAS 生产库（2000股@105.55）不匹配被拒绝启动。立即回滚 0.3.0 恢复服务。
- **修复（方案 A，已批准）**：锚点 1/2/3 改为 `MT_RECON_ANCHORS=1` 显式开启、默认关闭；schema 级不变量（FK / tenant 归因 / 孤儿租户）保持无条件强制。v122 为只读检查，checksum 变化导致的全库重跑幂等无害。
- 0.4.1：本地 828 项测试全过 → CI success → NAS 手动恢复 `:latest` 升级验证：healthy、v122 success=1、持仓数据完好。
- 飞书通知已发送（om_x100b693a1e47b938b1d1b3900c91c42）。
- 经验：① 发布前对"存量库迁移"类改动应在真实数据副本上预演；② tag push 未触发流水线（原因待查），目前用 workflow_dispatch 手动触发。
