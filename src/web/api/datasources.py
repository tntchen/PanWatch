...
# ── D5 config 凭证掩码（docs/19 D5 / docs/27 §行31）────────────────────────
# 与 providers.py 同一防御式范式：tenant_context 不可用时退化为不加掩码
# （等价单租户/现状明文），单租户直通短路。
try:  # 防御：tenant_context 不可用时退化为不掩码（等价单租户）
...
def _mt_ctx():
    """多租户模式下的当前租户 ctx；单租户直通/无 ctx 返回 None（等价现状明文）。"""
...
def _mask_config_for_non_admin() -> bool:
    """多租户且当前身份非 admin → config 掩码。

    ctx 缺失（公开路由/裸脚本）与 providers.py 口径一致：保守明文、等价现状；
    ctx 存在但 role 缺失时保守掩码（凭证宁可不出网）。
    """
    ctx = _mt_ctx()
    if ctx is None:
        return False
...
def _to_response(source: DataSource, health_map: dict | None = None) -> dict:
    ...
    return {
        ...
        "config": {} if _mask_config_for_non_admin() else (source.config or {}),
