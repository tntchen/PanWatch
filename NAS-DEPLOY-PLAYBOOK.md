...
### 案例 D：API 响应慢 ~28 秒——日志 Handler 持锁 I/O 冻结事件循环（0.4.2 真实事故）
症状：部署后所有 API 极慢，连空的 `/api/health` 都要 ~28 秒。
排查顺序（推荐）：
1. **先排除网络/数据源**：逐一对上游数据端点从 NAS 直连 curl 测速。本例 NAS 的 IP 被东方财富秒拒（TCP RST，Mac 正常）→ 禁用对应数据源；新浪汇率间歇超时 → 走代理（SSH 隧道把 Mac 的 Clash 7897 暴露给 NAS，.env 配 HTTP_PROXY）。
2. **网络修完仍慢 → 查应用内锁**：逐线程栈/日志时间戳定位。本例根因是 DBLogHandler 在 `emit()` 持锁期间内联 flush 写 SQLite，Timer flush 因 SQLite 写锁阻塞时，事件循环线程的日志 emit 全部卡在锁上 → 事件循环冻结。
教训与规则：
1. **日志 Handler 是隐形杀手**：任何 Handler 在持锁区做同步 I/O（尤其 SQLite 写），一旦 I/O 阻塞，会把"打一条日志"这种看似无害的操作变成全事件循环的冻结点。规则：emit 只做内存缓冲 append，I/O 全部移出持锁区、由后台线程统一执行。
2. **SQLite 写锁会级联放大**：引擎"写库未 commit、继续网络抓取"的模式长持写锁，busy_timeout 30s 意味着任何其他写者最长排队 30 秒——低流量时看不出来，一旦有别处（如日志）同步写库就雪崩。
3. **自建/NAS 出网可能被个别数据源封 IP**：TCP RST 级拒绝、时好时坏，排查时按数据源逐一 curl 对比"NAS 直连 vs Mac 直连 vs 代理"，禁源前先确认其他源可替代。
4. **SSH 隧道代理无自启动**，NAS 重启后需重建——写进运维清单或做成群晖计划任务。
